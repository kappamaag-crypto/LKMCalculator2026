...
COLUMNS = [
    ...
    ("Цена, ₽/кг", 95), ("DFT, мкм", 80), ("Потери, %", 75),
    ("Разб., %", 70), ("WFT, мкм", 80), ("Расход, л/м²", 105),
    ("Расход, кг/м²", 110), ("Стоимость, руб/м²", 125),
]
...
class LayerTableWidget(QTableWidget):
    ...
    def _dft_hint(material):
        ...
        if minimum is not None and maximum is not None:
            return f"Рекомендуемый DFT: {minimum:g}–{maximum:g} мкм."
        if minimum is not None:
            return f"Рекомендуемый DFT: от {minimum:g} мкм."
        if maximum is not None:
            return f"Рекомендуемый DFT: до {maximum:g} мкм."
        if hard_max is not None:
            return f"Жёсткий максимум DFT: {hard_max:g} мкм."
    ...
    def _dft_state(material, dft):
        ...
        maximum = getattr(material, "recommended_dft_max", None)
        hard_max = getattr(material, "hard_max_dft", None)
        if hard_max is not None and dft > hard_max:
            return "error", f"DFT {dft:g} мкм превышает жёсткий максимум {hard_max:g} мкм."
        if minimum is not None and dft < minimum:
            return "warning", f"DFT {dft:g} мкм ниже рекомендуемого минимума {minimum:g} мкм."
        if maximum is not None and dft > maximum:
            return "warning", f"DFT {dft:g} мкм выше рекомендуемого максимума {maximum:g} мкм."
    ...
    def set_layers(self, layers, results=None):
        ...
        try:
            ...
            for row, layer in enumerate(layers):
                ...
                self._fill(row, layer, result)
        finally:
            self._updating = False

    def _fill(self, row, layer, result):
        material = layer.material
        self.setItem(row, 0, self._item(row + 1))
        self.setItem(row, 1, self._item(material.material_name, Qt.AlignLeft | Qt.AlignVCenter))
        ...
        if result:
            cost = (
                f"{result.cost_per_m2 + result.thinner_cost_per_m2:.2f}"
                if result.cost_per_m2 is not None and result.thinner_cost_per_m2 is not None
                else "—"
            )
            values = (
                f"{result.wft:.1f}", f"{result.practical_consumption_l:.4f}",
                f"{result.practical_consumption_kg:.4f}", cost,
            )
        else:
            values = ("—", "—", "—", "—")
        for column, value in zip((8, 9, 10, 11), values):
            self.setItem(row, column, self._item(value))

    def _error(self, item, message):
        item.setToolTip(message)
        item.setData(Qt.UserRole, "invalid")
        item.setBackground(self.palette().brush(self.palette().ColorRole.BrightText))
        QToolTip.showText(self.viewport().mapToGlobal(self.visualItemRect(item).center()), message, self)
    ...
    def _restore_invalid(self, item, message):
        ...
        old = self._last_valid.get((item.row(), item.column()), "")
        QTimer.singleShot(700, lambda: self._restore_if_still_invalid(item, old))
    ...
    def _changed(self, item):
        ...
        if column == 4 and text == "":
            ...
            layer.material = replace(layer.material, price_per_kg=None, price_per_liter=None)
            self._last_valid[(row, column)] = ""
            self._clear_error(item)
        ...
        try:
            ...
            self._restore_invalid(item, "Введите число. Для цены можно оставить поле пустым — цена будет считаться неизвестной.")
            return
        rules = {
            4: (value >= 0, "Цена не может быть отрицательной."),
            5: (0 < value <= 2000, "DFT должен быть больше 0 и не превышать 2000 мкм."),
            6: (0 <= value <= 99, "Потери должны быть от 0 до 99 %."),
            7: (0 <= value <= 100, "Разбавитель должен быть от 0 до 100 %."),
        }
        valid, message = rules[column]
        if not valid:
            self._restore_invalid(item, message)
            return
        layer = self._layer_inputs[row]
        if column == 4:
            layer.material = replace(layer.material, price_per_kg=value, price_per_liter=None)
        elif column == 5:
            layer.target_dft = value
            state, hint = self._dft_state(layer.material, value)
            item.setData(Qt.UserRole, state)
            item.setToolTip(hint)
        elif column == 6:
            layer.losses_percent = value
        elif column == 7:
            layer.thinner_percent = value
    ...
    def add_layer(self, layer):
        self._layer_inputs.append(layer)
        row = self.rowCount()
        self.insertRow(row)
        self._updating = True
        try:
            self._fill(row, layer, None)
        finally:
            self._updating = False
        self.layer_changed.emit()
    ...
    def remove_selected(self):
        ...
        for index in range(self.rowCount()):
            self.item(index, 0).setText(str(index + 1))
        self._last_valid.clear()
        for index in range(self.rowCount()):
            for column in EDITABLE_COLUMNS:
                self._last_valid[(index, column)] = self.item(index, column).text()
        self.layer_changed.emit()
