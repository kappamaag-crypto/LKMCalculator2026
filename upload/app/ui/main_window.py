            ...
            for orm in rows:
                ...
                for layer in sorted(orm.layers,key=lambda x:x.layer_number):
                    ...
                    layers.append(LayerDefinition(
                        ...
                        layer_type=_enum_or_none(MaterialType,layer.layer_type) or MaterialType.OTHER,
                        dft_min=layer.dft_min,
                        dft_max=layer.dft_max,
                        target_dft=layer.target_dft if layer.target_dft is not None else dft_fallback(material),
                        thinner_percent=layer.thinner_percent or 0,
                        thinner_material_id=layer.thinner_material_id,
                        thinner_basis=layer.thinner_basis,
                        losses_percent=None,
                        notes=layer.notes or "",
                    ))
                if layers:
                    systems.append(CoatingSystem(
                        id=orm.id,
                        system_name=orm.system_name,
                        manufacturer=orm.manufacturer or "",
                        description=orm.description or "",
                        durability=_enum_or_none(DurabilityLevel,orm.durability),
                        substrate=orm.substrate or "",
                        total_dft_min=orm.total_dft_min,
                        total_dft_target=orm.total_dft_target,
                        total_dft_max=orm.total_dft_max,
                        number_of_layers=len(layers),
                        temperature_min=orm.temperature_min,
                        temperature_max=orm.temperature_max,
                        standards=orm.standards or "",
                        certificate=orm.certificate or "",
                        technical_document=orm.technical_document or "",
                        notes=orm.notes or "",
                        layers=layers,
                    ))
...
def dft_fallback(material):
    value=getattr(material,"recommended_dft_min",None)
