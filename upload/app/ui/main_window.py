...
class MainWindow(QMainWindow):
    ...
    def _show_pre_application_check(self):
        ...
        if result is None:
            QMessageBox.information(self,"Pre-Application Check","Сначала выполните расчёт в разделе «Расчёт».")
    ...
    def _show_calculation_explanation(self):
        result=getattr(self,"_last_calculation_result",None)
        if result is None:
            QMessageBox.information(self,"Пояснение расчёта","Сначала выполните расчёт в разделе «Расчёт».")
            self.tabs.setCurrentWidget(self.calc_view)
            return
        report=self.calc_service.explain_calculation(result)
        ExplanationDialog(report,self).exec()
