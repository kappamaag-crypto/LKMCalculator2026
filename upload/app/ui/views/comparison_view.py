...
class ComparisonView(QWidget):
    ...
    def _build_ui(self):
        ...
        title=QLabel("Сравнение систем");title.setProperty("heading",True);root.addWidget(title)
        ...
        row=QHBoxLayout()
        self.btn_compare=QPushButton("Сравнить");self.btn_compare.clicked.connect(self._on_compare)
        self.btn_export=QPushButton("Экспорт сравнения");self.btn_export.setProperty("secondary",True);self.btn_export.setEnabled(False);self.btn_export.clicked.connect(self._on_export)
        self.btn_clear=QPushButton("Очистить");self.btn_clear.setProperty("secondary",True);self.btn_clear.clicked.connect(self._on_clear)
        ...
        self.table=QTableWidget(0,1);self.table.setHorizontalHeaderLabels(["Показатель"]);self.table.horizontalHeader().setSectionResizeMode(0,QHeaderView.Stretch);self.table.setAlternatingRowColors(True);self.table.setSelectionMode(QAbstractItemView.NoSelection);self.table.verticalHeader().setVisible(False);root.addWidget(self.table)
        self.lbl_legend=QLabel();self.lbl_legend.setProperty("subheading",True);self.lbl_legend.setWordWrap(True);root.addWidget(self.lbl_legend)

    def set_object(self,obj:ObjectData):self._object=obj

    def add_system(self,name,layers):
        if len(self._systems_data)>=10:
            QMessageBox.warning(self,"Лимит","Максимум 10 систем для сравнения");return
        self._systems_data.append((name or f"Система {len(self._systems_data)+1}",list(layers)))
        self._result_data.clear()
        self._last_comparison=None;self.btn_export.setEnabled(False);self.lbl_count.setText(f"Систем: {len(self._systems_data)}")

    def add_from_calculation(self,result):
        if len(self._systems_data)>=10:
            QMessageBox.warning(self,"Лимит","Максимум 10 систем для сравнения");return
        self._systems_data.append((result.system.system_name or f"Система {len(self._systems_data)+1}",[]))
    ...
    def _on_compare(self):
        if len(self._systems_data)<2:
            QMessageBox.warning(self,"Внимание","Добавьте не менее 2 систем");return
        try:
            if len(self._result_data)==len(self._systems_data):
                self._last_comparison=self.service.comparison.compare_results(self._object,self._result_data)
            else:
                self._last_comparison=self.service.compare_systems(self._object,self._systems_data)
        except ValueError as exc:
            QMessageBox.warning(self,"Расчёт невозможен",str(exc));return
    ...
    def _fill_table(self,comparison:ComparisonResult):
        ...
        headers=["Показатель"]+[(s.system.system_name or f"Система {i+1}")[:40] for i,s in enumerate(systems)]
        ...
        for r,(label,values,highlight) in enumerate(rows):
            ...
            mn=min(numeric) if numeric else None;mx=max(numeric) if numeric else None
