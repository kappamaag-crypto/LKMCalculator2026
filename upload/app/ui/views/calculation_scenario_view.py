...
class CalculationScenarioView(QWidget):
    """Scenario UI; arithmetic remains in CalculationService."""
    def __init__(self,service:CalculationScenarioService,parent=None):
        super().__init__(parent);self.service=service;self._systems=[];self._context=EngineeringContext();self._results=[];self._build_ui()
    def _build_ui(self):
        ...
        sub=QLabel("Один объект и несколько альтернативных систем. Все альтернативы рассчитываются существующим CalculationService; складские и закупочные данные не используются.");sub.setWordWrap(True);sub.setProperty("subheading",True);root.addWidget(sub)
        box=QGroupBox("Объект сценария");form=QFormLayout(box)
        self.ed_name=QLineEdit("Сценарий сравнения");self.ed_object=QLineEdit();self.ed_customer=QLineEdit();self.ed_project=QLineEdit();self.spin_area=QDoubleSpinBox();self.spin_area.setRange(.01,1000000);self.spin_area.setValue(1);self.spin_area.setDecimals(2);self.spin_area.setSuffix(" м²")
        ...
        sb=QGroupBox("Альтернативы");sl=QVBoxLayout(sb);self.table_systems=QTableWidget(0,2);self.table_systems.setHorizontalHeaderLabels(["Использовать","Система"]);self.table_systems.setSelectionMode(QAbstractItemView.NoSelection);self.table_systems.horizontalHeader().setStretchLastSection(True);sl.addWidget(self.table_systems)
        ...
        rb=QGroupBox("Результаты");rl=QVBoxLayout(rb);self.table_result=QTableWidget(0,1);self.table_result.setHorizontalHeaderLabels(["Показатель"]);self.table_result.setSelectionMode(QAbstractItemView.NoSelection);self.table_result.horizontalHeader().setStretchLastSection(True);rl.addWidget(self.table_result);self.btn_cmp=QPushButton("Передать результаты в сравнение");self.btn_cmp.setEnabled(False);self.btn_cmp.clicked.connect(self._send_to_comparison);rl.addWidget(self.btn_cmp);root.addWidget(rb)
    def set_systems(self,systems:list[CoatingSystem]):
        ...
        self._results=[];self.table_result.setRowCount(0);self.btn_cmp.setEnabled(False)
    ...
    def _calculate(self):
        ...
        self._results=[x.result for x in out.alternatives];self._fill(self._results);self.btn_cmp.setEnabled(bool(self._results))
    @staticmethod
    def _fmt(v):return "—" if v is None else (f"{v:,.3f}".replace(","," ") if isinstance(v,float) else str(v))
    def _fill(self,results:list[SystemCalculationResult]):
        headers=["Показатель"]+[r.system.system_name[:40] for r in results];rows=[("Количество слоёв",[len(r.layers) for r in results]),("Общая толщина, мкм",[r.total_dft for r in results]),("Расход ЛКМ, кг/м²",[r.total_practical_consumption_kg for r in results]),("Расход ЛКМ, л/м²",[r.total_practical_consumption_l for r in results]),("Стоимость, руб/м²",[r.total_cost_per_m2 for r in results]),("Стоимость объекта, руб",[r.total_cost for r in results])]
