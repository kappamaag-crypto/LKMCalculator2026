...
class CalculationView(QWidget):
    ...
    def __init__(self, service: CalculationService, parent=None):
        ...
        self._restored_system_name = ""
        self._timer = QTimer(self)
        self._timer.setSingleShot(True)
        self._timer.setInterval(220)
        self._timer.timeout.connect(self._on_live_recalculate)
        self._build_ui()

    def _build_ui(self):
        root = QVBoxLayout(self); root.setContentsMargins(12, 12, 12, 12); root.setSpacing(10)
        t = QLabel("Расчёт системы покрытия"); t.setProperty("heading", True); root.addWidget(t)
        sp = QSplitter(Qt.Horizontal)

        left = QWidget(); ll = QVBoxLayout(left)
        b = QGroupBox("Объект"); f = QFormLayout(b)
        self.ed_object = QLineEdit(); self.ed_object.setPlaceholderText("Название объекта")
        self.ed_customer = QLineEdit(); self.ed_customer.setPlaceholderText("Заказчик")
        self.ed_project = QLineEdit(); self.ed_project.setPlaceholderText("Проект")
        self.ed_calc_number = QLineEdit(); self.ed_calc_number.setPlaceholderText("Номер расчёта")
        self.spin_area = QDoubleSpinBox(); self.spin_area.setRange(0, 1000000); self.spin_area.setValue(1); self.spin_area.setDecimals(2); self.spin_area.setSuffix(" м²")
        f.addRow("Объект:", self.ed_object); f.addRow("Заказчик:", self.ed_customer); f.addRow("Проект:", self.ed_project); f.addRow("№ расчёта:", self.ed_calc_number); f.addRow("Площадь:", self.spin_area)
        self.spin_area.valueChanged.connect(self._schedule_live_recalculate)
        ll.addWidget(b); ll.addStretch(); sp.addWidget(left)

        right = QWidget(); rl = QVBoxLayout(right)
        lb = QGroupBox("Слои системы"); al = QVBoxLayout(lb); row = QHBoxLayout()
        self.cmb_material = QComboBox(); self.cmb_material.setMinimumWidth(240)
        add = QPushButton("Добавить слой"); add.clicked.connect(self._on_add_layer)
        rem = QPushButton("Удалить"); rem.setProperty("secondary", True); rem.clicked.connect(self._on_remove_layer)
        clr = QPushButton("Очистить"); clr.setProperty("secondary", True); clr.clicked.connect(self._on_clear_layers)
        for x in (QLabel("Материал:"), self.cmb_material, add, rem, clr): row.addWidget(x)
        row.addStretch(); al.addLayout(row)
        h = QLabel("После добавления параметры DFT, потери, разбавитель и цена редактируются непосредственно в строке. Результаты обновляются автоматически."); h.setWordWrap(True); h.setProperty("subheading", True); al.addWidget(h)
        self.layer_table = LayerTableWidget(); self.layer_table.layer_changed.connect(self._schedule_live_recalculate); al.addWidget(self.layer_table); rl.addWidget(lb)

        buttons = QHBoxLayout()
        self.btn_calc = QPushButton("Рассчитать"); self.btn_calc.clicked.connect(self._on_calculate)
        self.btn_demo = QPushButton("Демо-система"); self.btn_demo.setProperty("secondary", True); self.btn_demo.clicked.connect(self._on_load_demo)
        self.btn_excel = QPushButton("Excel"); self.btn_excel.setProperty("secondary", True); self.btn_excel.clicked.connect(self._on_export_excel); self.btn_excel.setEnabled(False)
        self.btn_pdf = QPushButton("PDF"); self.btn_pdf.setProperty("secondary", True); self.btn_pdf.clicked.connect(self._on_export_pdf); self.btn_pdf.setEnabled(False)
        self.btn_to_cmp = QPushButton("В сравнение"); self.btn_to_cmp.setProperty("secondary", True); self.btn_to_cmp.clicked.connect(self._on_to_comparison); self.btn_to_cmp.setEnabled(False)
        for x in (self.btn_calc, self.btn_demo, self.btn_excel, self.btn_pdf, self.btn_to_cmp): buttons.addWidget(x)
        buttons.addStretch(); rl.addLayout(buttons)

        rb = QGroupBox("Результат"); rr = QVBoxLayout(rb); self.lbl_summary = QLabel("Выполните расчёт"); self.lbl_summary.setProperty("subheading", True); self.lbl_summary.setWordWrap(True); self.txt_details = QTextEdit(); self.txt_details.setReadOnly(True); self.txt_details.setMaximumHeight(210); rr.addWidget(self.lbl_summary); rr.addWidget(self.txt_details); rl.addWidget(rb)
        sp.addWidget(right); sp.setStretchFactor(0, 1); sp.setStretchFactor(1, 2); root.addWidget(sp)
    ...
    def _build_object_data(self):
        return ObjectData(
            ...
            calculation_number=self.ed_calc_number.text().strip(),
            area_m2=self.spin_area.value(),
        )

    def _build_system(self) -> CoatingSystem:
        return CoatingSystem(system_name=self._restored_system_name or "Пользовательская система")

    def _on_add_layer(self):
        m = self._current_material()
        if m is None:
            QMessageBox.warning(self, "Внимание", "Выберите материал"); return
    ...
    def _on_clear_layers(self):
        self._timer.stop(); self.layer_table.clear_layers(); self.lbl_summary.setText("Выполните расчёт"); self.txt_details.clear(); self._last_result = None; self._restored_system_name = ""
        for b in (self.btn_excel, self.btn_pdf, self.btn_to_cmp): b.setEnabled(False)

    def _on_load_demo(self):
        if len(self._materials) < 2:
            p = Material(manufacturer="Blank", brand="Blank", material_name="Грунт-Эмаль Blank Universal", material_type=MaterialType.PRIMER_ENAMEL, binder_type=BinderType.EPOXY, density=1.4, solids_percent=73, price_per_kg=552, recommended_dft_min=100, recommended_dft_max=200)
            f = Material(manufacturer="Blank", brand="Blank", material_name="Эмаль Blank Finish", material_type=MaterialType.FINISH, binder_type=BinderType.POLYURETHANE, density=1.3, solids_percent=58, price_per_kg=892, recommended_dft_min=60, recommended_dft_max=100)
            self.set_materials([p, f])
        else:
            p = self._materials[0]; f = self._materials[1] if len(self._materials) > 1 else p
    ...
    def _recalculate(self, dialogs=False, notify=False):
        ...
        if not layers:
            ...
            return False
        try:
            result, validation = self.service.calculate_system(self._build_object_data(), layers, system=self._build_system())
        ...
        if area > 0: self.lbl_summary.setToolTip(f"Разбавитель на объект: {thinner_l*area:.3f} л / {thinner_kg*area:.3f} кг")
    ...
    def restore_snapshot(self, snapshot: dict) -> bool:
        """Восстановить снимок истории в редактируемую форму."""
        if not isinstance(snapshot, dict):
            raise ValueError("Некорректный формат снимка истории")
        obj = snapshot.get("object") or {}
        self._timer.stop()
        self.ed_object.setText(str(obj.get("object_name") or "")); self.ed_customer.setText(str(obj.get("customer") or "")); self.ed_project.setText(str(obj.get("project") or "")); self.ed_calc_number.setText(str(obj.get("calculation_number") or ""))
        try: self.spin_area.setValue(float(obj.get("area_m2") or 0))
        except (TypeError, ValueError): self.spin_area.setValue(0)
        self._restored_system_name = str(snapshot.get("system_name") or "Пользовательская система")

        materials_by_id = {m.id: m for m in self._all_materials if m.id is not None}
        materials_by_name = {m.material_name: m for m in self._all_materials if m.material_name}
        layers = []
        missing = []
        for item in snapshot.get("layers") or []:
            ...
            if material is None:
                missing.append(str(item.get("material_name") or "без названия")); continue
            thinner = None
        ...
        for layer in layers: self.layer_table.add_layer(layer)
        self._last_result = None
        for b in (self.btn_excel, self.btn_pdf, self.btn_to_cmp): b.setEnabled(False)
        if not layers:
            raise ValueError("В снимке нет слоёв, которые удалось восстановить из текущей базы материалов.")
        ok = self._recalculate(dialogs=False, notify=False)
        if missing:
            QMessageBox.warning(self, "История", "Не удалось восстановить: " + ", ".join(missing))
