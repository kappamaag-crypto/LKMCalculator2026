...
class CalculationView(QWidget):
    ...
    def _build_ui(self):
        ...
        left=QWidget();ll=QVBoxLayout(left);b=QGroupBox("Объект");f=QFormLayout(b);self.ed_object=QLineEdit();self.ed_object.setPlaceholderText("Название объекта");self.ed_customer=QLineEdit();self.ed_customer.setPlaceholderText("Заказчик");self.ed_project=QLineEdit();self.ed_project.setPlaceholderText("Проект");self.ed_calc_number=QLineEdit();self.ed_calc_number.setPlaceholderText("Номер расчёта");self.spin_area=QDoubleSpinBox();self.spin_area.setRange(0.01,1000000);self.spin_area.setValue(1);self.spin_area.setDecimals(2);self.spin_area.setSuffix(" м²");f.addRow("Объект:",self.ed_object);f.addRow("Заказчик:",self.ed_customer);f.addRow("Проект:",self.ed_project);f.addRow("№ расчёта:",self.ed_calc_number);f.addRow("Площадь:",self.spin_area);self.spin_area.valueChanged.connect(self._on_area_changed);ll.addWidget(b);ll.addStretch();sp.addWidget(left)
    ...
    def _on_load_demo(self):
        ...
        self.layer_table.clear_layers();self.layer_table.add_layer(LayerInput(material=p,target_dft=200,losses_percent=5));self.layer_table.add_layer(LayerInput(material=f,target_dft=80,losses_percent=5));self.ed_object.setText("Резервуар РВС-1000 (демо)");self.ed_customer.setText("ООО Пример");self.ed_project.setText("");self.ed_calc_number.setText("");self.spin_area.setValue(1250);self._area_unknown=False;self._restored_system_name="Blank Universal + Finish"
