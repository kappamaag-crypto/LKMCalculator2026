...
class CalculationView(QWidget):
    ...
    def _build_ui(self):
        ...
        left=QWidget();ll=QVBoxLayout(left);b=QGroupBox("Объект");f=QFormLayout(b);self.ed_object=QLineEdit();self.ed_object.setPlaceholderText("Название объекта");self.ed_customer=QLineEdit();self.ed_customer.setPlaceholderText("Заказчик");self.ed_project=QLineEdit();self.ed_project.setPlaceholderText("Проект");self.ed_calc_number=QLineEdit();self.ed_calc_number.setPlaceholderText("Номер расчёта");self.spin_area=QDoubleSpinBox();self.spin_area.setRange(0.01,1000000);self.spin_area.setValue(1);self.spin_area.setDecimals(2);self.spin_area.setSuffix(" м²");f.addRow("Объект:",self.ed_object);f.addRow("Заказчик:",self.ed_customer);f.addRow("Проект:",self.ed_project);f.addRow("№ расчёта:",self.ed_calc_number);f.addRow("Площадь:",self.spin_area);self.spin_area.valueChanged.connect(self._schedule_live_recalculate);ll.addWidget(b);ll.addStretch();sp.addWidget(left)
    ...
    def restore_snapshot(self,snapshot:dict)->bool:
        ...
        area=self._snapshot_number(obj.get("area_m2"));self.spin_area.setValue(area if area is not None and area>=0.01 else 0.01)
