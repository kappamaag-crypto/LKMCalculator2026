...
class CalculationView(QWidget):
    ...
    def _recalculate(self,dialogs=False,notify=False):
        ...
        if validation.has_errors:
            ...
            return False
        self._last_result=result;thinner_l=sum(lr.thinner_consumption_l for lr in result.layers);thinner_kg=sum(lr.thinner_consumption_kg for lr in result.layers);area=result.object_data.area_m2;self.layer_table.set_layers(layers,result.layers);self.lbl_summary.setText(f"Толщина: {result.total_dft:.0f} мкм  |  ЛКМ: {result.total_practical_consumption_l:.4f} л/м² / {result.total_practical_consumption_kg:.3f} кг/м²  |  Разбавитель: {thinner_l:.4f} л/м² / {thinner_kg:.3f} кг/м²  |  Стоимость: {self._money(result.total_cost_per_m2)} руб/м²  |  Объект: {self._money(result.total_cost,0)} руб")
