...
class CalculationView(QWidget):
    ...
    def _build_ui(self):
        ...
        left=QWidget();ll=QVBoxLayout(left);b=QGroupBox("Объект");f=QFormLayout(b);self.ed_object=QLineEdit();self.ed_object.setPlaceholderText("Название объекта");self.ed_customer=QLineEdit();self.ed_customer.setPlaceholderText("Заказчик");self.ed_project=QLineEdit();self.ed_project.setPlaceholderText("Проект");self.ed_calc_number=QLineEdit();self.ed_calc_number.setPlaceholderText("Номер расчёта");self.ed_system_name=QLineEdit();self.ed_system_name.setPlaceholderText("Например: Blank Universal + Finish");self.spin_area=QDoubleSpinBox();self.spin_area.setRange(0.01,1000000);self.spin_area.setValue(1);self.spin_area.setDecimals(2);self.spin_area.setSuffix(" м²");f.addRow("Объект:",self.ed_object);f.addRow("Заказчик:",self.ed_customer);f.addRow("Проект:",self.ed_project);f.addRow("№ расчёта:",self.ed_calc_number);f.addRow("Система:",self.ed_system_name);f.addRow("Площадь:",self.spin_area);self.spin_area.valueChanged.connect(self._on_area_changed);ll.addWidget(b);ll.addStretch();sp.addWidget(left)
    ...
    def _on_area_changed(self):
        self._area_unknown=False
        self._schedule_live_recalculate()
    ...
    def _on_load_demo(self):
        ...
        self.layer_table.clear_layers();self.layer_table.add_layer(LayerInput(material=p,target_dft=200,losses_percent=5));self.layer_table.add_layer(LayerInput(material=f,target_dft=80,losses_percent=5));self.ed_object.setText("Резервуар РВС-1000 (демо)");self.ed_customer.setText("ООО Пример");self.ed_project.setText("");self.ed_calc_number.setText("");self.ed_system_name.setText("Blank Universal + Finish");self.spin_area.setValue(1250);self._area_unknown=False;self._restored_system_name="Blank Universal + Finish"
    ...
    def _snapshot_number(value):
        """Backward-compatible wrapper around the pure snapshot conversion helper."""
    ...
    def restore_snapshot(self,snapshot:dict)->bool:
        ...
        area=snapshot_number(obj.get("area_m2"));self.spin_area.setValue(area if area is not None and area>=0.01 else 0.01);self._area_unknown=area is None or area<0.01
        self._restored_system_name=str(snapshot.get("system_name") or "Пользовательская система");self.ed_system_name.setText(self._restored_system_name);materials_by_id={m.id:m for m in self._all_materials if m.id is not None};materials_by_name={m.material_name:m for m in self._all_materials if m.material_name};layers=[];missing=[]
        for index,item in enumerate(snapshot.get("layers") or [],1):
            current=materials_by_id.get(item.get("material_id")) or materials_by_name.get(item.get("material_name"));material=material_from_snapshot(item,current);thinner=None;thinner_raw=item.get("thinner_percent") if "thinner_percent" in item else 0;thinner_percent=snapshot_number(thinner_raw)
            if thinner_percent is not None and thinner_percent>0:
                ...
                if item.get("thinner_density") is None and current_thinner is None: missing.append(str(item.get("thinner_name") or "разбавитель"));continue
                thinner=thinner_from_snapshot(item,current_thinner)
            target_dft=snapshot_number(item.get("target_dft"));losses_raw=item.get("losses_percent") if "losses_percent" in item else 0;losses_percent=snapshot_number(losses_raw)
            if target_dft is None: missing.append(f"DFT слоя №{index} ({item.get('material_name') or 'материал не указан'})")
    ...
    def _on_to_comparison(self):
        if self._last_result is not None:self.add_to_comparison.emit(self._last_result)
