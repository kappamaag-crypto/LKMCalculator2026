...
def explain_pre_application(
    report: ExplanationReport,
    pre_app: "PreApplicationCheckResult",
) -> None:
    """Fold Pre-Application Check into ExplanationReport (no invention)."""
    ...
    if status == READY:
        level = OK
        code = "PREAPP_READY"
        msg = "Pre-Application: READY — ошибок и критичных UNKNOWN по условиям нанесения нет."
    elif status == BLOCKED:
        level = WARNING
        code = "PREAPP_BLOCKED"
        msg = "Pre-Application: BLOCKED — есть ошибки условий нанесения."
    else:
        level = UNKNOWN_DATA
        code = "PREAPP_INCOMPLETE"
        msg = "Pre-Application: INCOMPLETE — ключевые данные UNKNOWN, готовность не подтверждена."
    ...
    for item in pre_app.items:
        ...
        report.add(
            ...
            lvl,
            item.message,
            scope=item.scope or "object",
            material_name=item.material_name,
            field=item.field,
            source_note="PreApplicationItem",
        )


def explain_chemical_resistance(
    report: ExplanationReport,
    chem: "ChemicalResistanceCheckResult",
) -> None:
    """Fold chemical resistance outcomes into ExplanationReport.

    Empty registry / UNKNOWN outcomes stay UNKNOWN_DATA — never invent RESISTANT.
    """
    ...
    if not chem.outcomes:
        report.add(
            "CHEM_EMPTY",
            UNKNOWN_DATA,
            "Химстойкость: нет исходов (пустой registry / нет KNOWN rules) → UNKNOWN.",
            scope="context",
            source_note="ChemicalResistanceCheckResult",
        )
        return
    ...
    for mat_name, by_agent in sorted(chem.outcomes.items()):
        for agent_id, outcome in sorted(by_agent.items()):
            ...
            report.add(
                code,
                lvl,
                f"Химстойкость: {mat_name} × {agent_id} → {outcome}.",
                scope="material",
                material_name=mat_name,
                field=agent_id,
                source_note="ChemicalResistanceCheckResult (source-backed only)",
            )


def explain_recommendation_signals(
    report: ExplanationReport,
    *,
    ...
    """Surface recommendation reasons/warnings/limitations as explanation items."""
    for i, text in enumerate(reasons or ()):
        ...
        report.add(
            ...
            str(text).strip(),
            scope="system",
            source_note="RecommendationItem.reasons",
        )
    for i, text in enumerate(warnings or ()):
        ...
        report.add(
            ...
            str(text).strip(),
            scope="system",
            source_note="RecommendationItem.warnings",
        )
    for i, text in enumerate(limitations or ()):
        ...
        report.add(
            ...
            str(text).strip(),
            scope="system",
            source_note="RecommendationItem.limitations",
        )
...
def explain_engineering_bundle(
    ...
    *,
    ...
    """System calculation explanation plus optional pre-app / chem / recommendation signals.

    Integration slice for §26: no invented values; optional inputs only fold when provided.
    """
    ...
    if recommendation_reasons or recommendation_warnings or recommendation_limitations:
        explain_recommendation_signals(
            report,
