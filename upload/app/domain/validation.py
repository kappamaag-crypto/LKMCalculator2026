...
from app.domain.models import Material, LayerDefinition, LayerResult, CoatingSystem, ObjectData, SystemCalculationResult
...
def validate_material(material: Material) -> ValidationResult:
    ...
    if material.density is None:
        result.add_error("MAT_DENSITY_UNKNOWN", "Плотность материала неизвестна — расчёт массы в кг невозможен", "density")
    elif material.density <= 0:
        result.add_error("MAT_DENSITY_INVALID", "Плотность материала должна быть больше нуля", "density")

    if material.solids_by_volume_percent is None:
        if material.solids_percent is None:
            result.add_error("MAT_SOLIDS_UNKNOWN", "Объёмная доля сухого остатка неизвестна — инженерный расчёт WFT невозможен", "solids_by_volume_percent")
        elif material.solids_percent <= 0 and material.material_type.value != "разбавитель":
            result.add_error("MAT_SOLIDS_ZERO", "Сухой остаток равен 0 % — расчёт WFT невозможен", "solids_percent")
        else:
            result.add_warning(
                "MAT_SOLIDS_BASIS_LEGACY",
                "Объёмная доля сухого остатка не указана; расчёт использует legacy-поле «solids_percent». Проверьте базу значения по TDS.",
                "solids_by_volume_percent",
            )
    else:
        if material.solids_by_volume_percent < 0 or material.solids_by_volume_percent > 100:
            result.add_error("MAT_SV_RANGE", "Сухой остаток по объёму должен быть в диапазоне 0–100 %", "solids_by_volume_percent")
        elif material.solids_by_volume_percent == 0 and material.material_type.value != "разбавитель":
            result.add_error("MAT_SV_ZERO", "Сухой остаток по объёму равен 0 % — расчёт WFT невозможен", "solids_by_volume_percent")

    if material.solids_percent is not None:
        if material.solids_percent < 0 or material.solids_percent > 100:
            result.add_error("MAT_SOLIDS_RANGE", "Сухой остаток должен быть в диапазоне 0–100 %", "solids_percent")
    ...
    _validate_ordered_range(result, material.min_application_temperature, material.max_application_temperature,
                            "MAT_TEMP_RANGE", "Диапазон температуры нанесения задан некорректно", "application_temperature")
    _validate_ordered_range(result, material.min_recoat_time_h, material.max_recoat_time_h,
                            "MAT_RECOAT_RANGE", "Диапазон межслойной выдержки задан некорректно", "recoat_time_h")
    ...
    if material.recommended_dft_min is not None and material.recommended_dft_max is not None and material.recommended_dft_min > material.recommended_dft_max:
        result.add_error("MAT_DFT_RANGE", "Минимальная рекомендуемая толщина больше максимальной", "recommended_dft")
    ...
    if material.thinner_percent_min is not None and material.thinner_percent_max is not None and material.thinner_percent_min > material.thinner_percent_max:
    ...
    if material.thinner_basis not in VALID_THINNER_BASES:
        result.add_error("MAT_THINNER_BASIS", f"Неизвестная база расчёта разбавления: «{material.thinner_basis}»", "thinner_basis")

    return result


def _validate_ordered_range(result: ValidationResult, minimum: Optional[float], maximum: Optional[float], code: str, message: str, field: str) -> None:
    if minimum is not None and maximum is not None and minimum > maximum:
        result.add_error(code, message, field)


def validate_layer_input(material: Material, target_dft: float, losses_percent: float = 0.0,
                         thinner_percent: float = 0.0, layer_index: int | None = None,
                         thinner_basis: str | None = None) -> ValidationResult:
    ...
    if thinner_percent >= 100:
        result.add_error("LAYER_THINNER_GE100", "Процент разбавителя должен быть меньше 100 %", "thinner_percent", layer_index)
    ...
    if material.thinner_required and thinner_percent <= 0:
        result.add_warning("LAYER_THINNER_REQUIRED", f"Для «{material.material_name}» указан обязательный разбавитель, но процент разбавления не задан", "thinner_percent", layer_index)
    if material.thinner_percent_min is not None and thinner_percent < material.thinner_percent_min:
        result.add_warning("LAYER_THINNER_BELOW_MIN", f"Разбавление {thinner_percent:g} % ниже рекомендованного минимума {material.thinner_percent_min:g} %", "thinner_percent", layer_index)
    if material.thinner_percent_max is not None and thinner_percent > material.thinner_percent_max:
        result.add_error("LAYER_THINNER_ABOVE_MAX", f"Разбавление {thinner_percent:g} % превышает допустимый максимум {material.thinner_percent_max:g} %", "thinner_percent", layer_index)
    ...
    if material.recommended_dft_min is not None and target_dft > 0 and target_dft < material.recommended_dft_min:
        result.add_warning("LAYER_DFT_BELOW_MIN", f"Толщина {target_dft} мкм ниже рекомендуемого минимума ({material.recommended_dft_min} мкм) для «{material.material_name}»", "target_dft", layer_index)
    if material.recommended_dft_max is not None and target_dft > 0 and target_dft > material.recommended_dft_max:
        result.add_warning("LAYER_DFT_ABOVE_MAX", f"Толщина {target_dft} мкм выше рекомендуемого максимума ({material.recommended_dft_max} мкм) для «{material.material_name}»", "target_dft", layer_index)
    if material.max_single_layer_dft is not None and target_dft > material.max_single_layer_dft:
        result.add_error("LAYER_DFT_MAX_SINGLE", f"Толщина {target_dft} мкм превышает максимальную толщину одного слоя ({material.max_single_layer_dft} мкм) для «{material.material_name}»", "target_dft", layer_index)
    return result
...
def validate_application_conditions(obj: ObjectData, material: Material, layer_index: int | None = None) -> ValidationResult:
    ...
    if temperature is not None:
        if material.min_application_temperature is not None and temperature < material.min_application_temperature:
            result.add_error("COND_TEMP_BELOW_MIN", f"Температура {temperature:g} °C ниже минимальной для «{material.material_name}» ({material.min_application_temperature:g} °C)", "surface_temperature", layer_index)
        if material.max_application_temperature is not None and temperature > material.max_application_temperature:
            result.add_error("COND_TEMP_ABOVE_MAX", f"Температура {temperature:g} °C выше максимальной для «{material.material_name}» ({material.max_application_temperature:g} °C)", "surface_temperature", layer_index)
    if obj.relative_humidity is not None and material.max_relative_humidity is not None and obj.relative_humidity > material.max_relative_humidity:
        result.add_error("COND_RH_ABOVE_MAX", f"Относительная влажность {obj.relative_humidity:g} % превышает допустимую для «{material.material_name}» ({material.max_relative_humidity:g} %)", "relative_humidity", layer_index)
    if obj.surface_temperature is not None and obj.dew_point is not None:
        ...
        if margin < required:
            result.add_error("COND_DEW_MARGIN", f"Запас температуры поверхности до точки росы {margin:.1f} °C меньше требуемого {required:.1f} °C для «{material.material_name}»", "dew_point_margin_c", layer_index)
    return result
...
def validate_object_data(obj: ObjectData) -> ValidationResult:
    ...
    if obj.relative_humidity is not None and not 0 <= obj.relative_humidity <= 100:
    ...
    if obj.surface_temperature is not None and obj.dew_point is not None and obj.surface_temperature <= obj.dew_point:
        result.add_error("OBJ_DEW_POINT", f"Температура поверхности ({obj.surface_temperature} °C) не выше точки росы ({obj.dew_point} °C) — нанесение недопустимо", "dew_point")
...
def validate_system_layers(layers: Sequence[LayerDefinition | LayerResult], compatibility_checker=None, system: CoatingSystem | None = None) -> ValidationResult:
    ...
    for i, layer in enumerate(layers):
        if isinstance(layer, LayerResult):
            ...
            material = layer.material
            binder = material.binder_type.value if material and hasattr(material.binder_type, "value") else (str(material.binder_type) if material else "")
            dft = layer.target_dft
        ...
        if compatibility_checker and prev_binder and binder:
            ...
            if status == CompatibilityStatus.FORBIDDEN.value:
                result.add_error("SYS_COMPAT_FORBIDDEN", f"Слои {i}/{i + 1}: сочетание связующих «{prev_binder}» → «{binder}» запрещено", layer_index=i)
            elif status == CompatibilityStatus.WARNING.value:
                result.add_warning("SYS_COMPAT_WARNING", f"Слои {i}/{i + 1}: сочетание «{prev_binder}» → «{binder}» требует проверки", layer_index=i)
            elif status == CompatibilityStatus.UNKNOWN.value:
                result.add_info("SYS_COMPAT_UNKNOWN", f"Слои {i}/{i + 1}: нет подтвержденных данных о совместимости", layer_index=i)
        if binder:
            prev_binder = binder

    if system:
        if system.number_of_layers and system.number_of_layers != len(layers):
            result.add_error("SYS_LAYER_COUNT", f"Система требует {system.number_of_layers} слоёв, задано {len(layers)}", "number_of_layers")
        if system.total_dft_min is not None and total_target_dft < system.total_dft_min:
            result.add_error("SYS_DFT_BELOW_MIN", f"Суммарная DFT {total_target_dft:g} мкм ниже минимума системы {system.total_dft_min:g} мкм", "total_dft")
        if system.total_dft_max is not None and total_target_dft > system.total_dft_max:
            result.add_error("SYS_DFT_ABOVE_MAX", f"Суммарная DFT {total_target_dft:g} мкм выше максимума системы {system.total_dft_max:g} мкм", "total_dft")
...
def validate_before_calculation(obj: ObjectData, layers: Sequence[tuple[Material, float, float, float]], compatibility_checker=None, system: CoatingSystem | None = None) -> ValidationResult:
    """Единая проверка перед расчётом."""
    result = validate_object_data(obj)
    defs: list[LayerDefinition] = []
    for i, (material, dft, losses, thinner) in enumerate(layers):
        result.merge(validate_layer_input(material, dft, losses, thinner, i))
        result.merge(validate_application_conditions(obj, material, i))
        defs.append(LayerDefinition(material=material, target_dft=dft, losses_percent=losses, thinner_percent=thinner))
    result.merge(validate_system_layers(defs, compatibility_checker, system=system))
    return result
