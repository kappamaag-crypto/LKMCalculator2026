...
from app.domain.loss_profile import LossProfile, ResolvedLosses
...
class LayerCalculator:
    ...
    def calculate(
        ...
        result = LayerResult(
            ...
            losses_profile_name=losses_profile_name,
            losses_note=losses_note,
            thinner_percent=thinner_percent,
            thinner=thinner,
            thinner_basis=basis,
            wft=calc.wft,
...
class SystemCalculator:
    ...
    def resolve_losses(
        ...
        Priority:
        1. explicit_percent when provided (including 0.0) → EXPLICIT
        2. layer_profile, else calculator loss_profile → PROFILE
        3. default_losses (legacy, defaults to 0.0 — not a hidden engineering value) → DEFAULT
        """
        profile = layer_profile if layer_profile is not None else self.loss_profile
        if explicit_percent is not None:
            if profile is not None:
                percent = profile.resolve(explicit_percent)
            else:
                LossProfile._validate_percent(explicit_percent)
                percent = explicit_percent
            return ResolvedLosses(
                percent=percent,
                source="EXPLICIT",
                profile_name=profile.name if profile is not None else "",
                note=(profile.note or "") if profile is not None else "LayerInput.losses_percent",
            )
        if profile is not None:
            return ResolvedLosses(
                percent=profile.resolve(None),
                source="PROFILE",
                profile_name=profile.name,
                note=profile.note or f"LossProfile source={profile.source}",
            )
        LossProfile._validate_percent(self.default_losses)
        return ResolvedLosses(
            percent=self.default_losses,
            source="DEFAULT",
            note="legacy default_losses (0.0 is not a hidden engineering coefficient)",
        )
    ...
    def calculate(
        ...
        resolved_losses: list[ResolvedLosses] = []
        ...
        layer_results = [
            LayerCalculator.calculate(
                ...
                area,
                li.thinner_basis,
                losses_source=resolved_losses[idx].source,
                losses_profile_name=resolved_losses[idx].profile_name,
                losses_note=resolved_losses[idx].note,
            )
            for idx, li in enumerate(layer_inputs)
        ]
