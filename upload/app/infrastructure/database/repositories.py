...
def material_orm_to_domain(orm: MaterialORM) -> Material:
    return Material(
        ...
        density=orm.density or 0.0,
        solids_percent=orm.solids_percent or 0.0,
        voc=orm.voc,
        color=orm.color or "",
        ral=orm.ral or "",
        price_per_kg=orm.price_per_kg,
        price_per_liter=orm.price_per_liter,
        ...
        packaging_kg=orm.packaging_kg,
        packaging_l=orm.packaging_l,
        is_active=orm.is_active,
        is_incomplete=orm.is_incomplete,
        notes=orm.notes or "",
        datasheet=orm.datasheet or "",
...
class MaterialRepository:
    ...
    def get_by_name(self, name: str) -> Optional[Material]:
        stmt = select(MaterialORM).where(MaterialORM.material_name == name)
    ...
    def list_materials(self, active_only: bool = True) -> list[Material]:
        """Материалы без разбавителей."""
        stmt = (
            select(MaterialORM)
            .where(MaterialORM.material_type != MaterialType.THINNER.value)
    ...
    def search(self, query: str, active_only: bool = True) -> list[Material]:
        ...
        stmt = (
            ...
            .where(
                or_(
                    MaterialORM.material_name.ilike(q),
                    MaterialORM.manufacturer.ilike(q),
                    MaterialORM.brand.ilike(q),
                    MaterialORM.binder_type.ilike(q),
                    MaterialORM.ral.ilike(q),
                )
            )
            .order_by(MaterialORM.material_name)
        )
...
class CoatingSystemRepository:
    ...
    def search(self, query: str, active_only: bool = True) -> Sequence[CoatingSystemORM]:
        ...
        stmt = (
            ...
            .where(
                or_(
                    CoatingSystemORM.system_name.ilike(q),
                    CoatingSystemORM.manufacturer.ilike(q),
