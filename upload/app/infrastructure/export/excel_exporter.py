...
class ExcelExporter:
    ...
    def export_comparison(self, comparison: ComparisonResult, path: str | Path) -> Path:
        ...
        for i, result in enumerate(comparison.systems):
            name = (result.system.system_name or f"Система {i + 1}")[:28]
            self._sheet_layers(wb.create_sheet(name), result)
        wb.save(path)
        return path
    ...
    def _title_block(self, ws: Worksheet, title: str, obj: ObjectData, start_row: int = 1) -> int:
        ...
        row = start_row + 1
        ws.cell(row, 1, self.settings.organization_name).font = SUBTITLE_FONT
        row += 1
        ws.cell(row, 1, f"Дата: {datetime.now().strftime('%d.%m.%Y %H:%M')}").font = NORMAL_FONT
        row += 1
    ...
    def _sheet_input(self, ws: Worksheet, result: SystemCalculationResult) -> None:
        ...
        row = self._title_block(ws, "Расчёт расхода ЛКМ / подбор системы АКЗ", result.object_data)
        ws.cell(row, 1, "Условия эксплуатации").font = SUBTITLE_FONT
        row += 1
        fields = [
            ("Категория коррозии", result.object_data.corrosion_category.value if result.object_data.corrosion_category else "—"),
            ("Долговечность", result.object_data.durability.value if result.object_data.durability else "—"),
            ("Поверхность", result.object_data.surface_type.value if result.object_data.surface_type else "—"),
            ("Среда", result.object_data.environment.value if result.object_data.environment else "—"),
            ("T мин, °C", result.object_data.temperature_min if result.object_data.temperature_min is not None else "—"),
            ("T макс, °C", result.object_data.temperature_max if result.object_data.temperature_max is not None else "—"),
            ("Система", result.system.system_name or "Пользовательская"),
        ]
        for label, value in fields:
            _cell(ws, row, 1, label, True, align=LEFT)
            _cell(ws, row, 2, value, align=LEFT)
            row += 1
        _auto_width(ws)

    def _sheet_layers(self, ws: Worksheet, result: SystemCalculationResult) -> None:
        ws.title = "Слои" if ws.title == "Sheet" else ws.title
        ws.cell(1, 1, f"Слои системы: {result.system.system_name}").font = TITLE_FONT
        row = 3
        headers = ["№", "Материал", "Связующее", "DFT, мкм", "WFT, мкм", "Потери, %", "Разб., %", "Укрыв. теор., м²/л", "Укрыв. практ., м²/л", "Расход теор., кг/м²", "Расход практ., кг/м²", "Расход практ., л/м²", "Стоимость, руб/м²", "Количество на объект, кг", "Стоимость на объект, руб"]
        ...
        _cell(ws, row, 13, result.total_cost_per_m2, True, TOTAL_FILL)
        _cell(ws, row, 14, result.total_practical_consumption_kg * (result.object_data.area_m2 or 0), True, TOTAL_FILL)
        _cell(ws, row, 15, result.total_cost, True, TOTAL_FILL)
        ws.freeze_panes = "A4"
        _auto_width(ws, 8, 28)

    def _sheet_materials(self, ws: Worksheet, result: SystemCalculationResult) -> None:
        ws.cell(1, 1, "Спецификация материалов").font = TITLE_FONT
        row = 3
        _write_header_row(ws, row, ["Материал", "Производитель", "Плотность, кг/л", "Сухой остаток, %", "Цена, руб/кг", "Цена, руб/л", "Количество, кг", "Количество, л", "Стоимость, руб"])
        row += 1
        for i, lr in enumerate(result.layers):
            fill = ALT_FILL if i % 2 else None
    ...
    def _sheet_comparison(self, ws: Worksheet, comparison: ComparisonResult) -> None:
        ...
        _write_header_row(ws, row, ["Показатель"] + [(s.system.system_name or f"Система {i + 1}")[:25] for i, s in enumerate(systems)])
        row += 1
    ...
    def _sheet_recommendations(self, ws: Worksheet, recommendation: RecommendationResult) -> None:
        ws.cell(1, 1, "Рекомендации по системам").font = TITLE_FONT
        row = 3
        _write_header_row(ws, row, ["Место", "Система", "Статус", "Баллы", "Причины / ограничения"])
        row += 1
        for item in recommendation.items:
            reasons = "; ".join(item.reasons + item.warnings + item.limitations)
            for col, value in enumerate([item.rank, item.system.system_name, item.status, item.score, reasons], 1):
                _cell(ws, row, col, value, align=LEFT if col in (2, 5) else CENTER)
            row += 1
        _auto_width(ws)
