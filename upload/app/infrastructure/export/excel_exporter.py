...
class ExcelExporter:
    ...
    def export_comparison(self,comparison:ComparisonResult,path:str|Path)->Path:
        ...
        for i,result in enumerate(comparison.systems):
            self._sheet_layers(wb.create_sheet((result.system.system_name or f"Система {i+1}")[:28]),result)
        wb.save(path)
        return path
    ...
    def _sheet_input(self,ws:Worksheet,result:SystemCalculationResult)->None:
        ...
        ws.cell(row,1,"Параметры расчёта").font=SUBTITLE_FONT
        row+=1
        fields=[
    ...
    def _sheet_layers(self,ws:Worksheet,result:SystemCalculationResult)->None:
        ...
        for i,lr in enumerate(result.layers):
            ...
            values=[
                i+1,lr.material.material_name,binder,lr.target_dft,lr.wft,lr.losses_percent,lr.thinner_percent,
                lr.theoretical_coverage,lr.practical_coverage,lr.theoretical_consumption_kg,
                lr.practical_consumption_kg,lr.practical_consumption_l,lr.cost_per_m2,
                lr.thinner_cost_per_m2,lr.cost_per_m2+lr.thinner_cost_per_m2,
    ...
    def _sheet_materials(self,ws:Worksheet,result:SystemCalculationResult)->None:
        ...
        _write_header_row(ws,row,[
            "Материал","Производитель","Плотность, кг/л","Сухой остаток, %","Цена, руб/кг",
    ...
    def _sheet_comparison(self,ws:Worksheet,comparison:ComparisonResult)->None:
        ...
        rows=[
            ...
            ("Практический расход, кг/м²",[s.total_practical_consumption_kg for s in systems]),
            ("Практический расход, л/м²",[s.total_practical_consumption_l for s in systems]),
