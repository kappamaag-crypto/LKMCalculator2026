...
def _auto_width(ws:Worksheet,min_width:int=10,max_width:int=40)->None:
    for col in ws.columns:
        letter=get_column_letter(col[0].column); length=max((len(str(c.value or "")) for c in col),default=min_width); ws.column_dimensions[letter].width=min(max(length+2,min_width),max_width)
...
class ExcelExporter:
    ...
    def _title_block(self,ws:Worksheet,title:str,obj:ObjectData,start_row:int=1)->int:
        ws.cell(start_row,1,title).font=TITLE_FONT; ws.merge_cells(start_row=start_row,start_column=1,end_row=start_row,end_column=6); row=start_row+1; ws.cell(row,1,self.settings.organization_name).font=SUBTITLE_FONT; row+=1; ws.cell(row,1,f"Дата: {datetime.now().strftime('%d.%m.%Y %H:%M')}").font=NORMAL_FONT; row+=1
        ...
        area=obj.area_m2 if obj.area_m2>0 else obj.area_per_element*obj.elements_count; ws.cell(row,1,f"Площадь: {area:.2f} м²").font=NORMAL_FONT; return row+2
    def _sheet_input(self,ws:Worksheet,result:SystemCalculationResult)->None:
        ws.title="Исходные данные"; row=self._title_block(ws,"Расчёт расхода ЛКМ / подбор системы АКЗ",result.object_data); ws.cell(row,1,"Условия эксплуатации").font=SUBTITLE_FONT; row+=1
        fields=[("Категория коррозии",result.object_data.corrosion_category.value if result.object_data.corrosion_category else "—"),("Долговечность",result.object_data.durability.value if result.object_data.durability else "—"),("Поверхность",result.object_data.surface_type.value if result.object_data.surface_type else "—"),("Среда",result.object_data.environment.value if result.object_data.environment else "—"),("T мин, °C",result.object_data.temperature_min if result.object_data.temperature_min is not None else "—"),("T макс, °C",result.object_data.temperature_max if result.object_data.temperature_max is not None else "—"),("Система",result.system.system_name or "Пользовательская")]
    ...
    def _sheet_layers(self,ws:Worksheet,result:SystemCalculationResult)->None:
        ...
        headers=["№","Материал","Связующее","DFT, мкм","WFT, мкм","Потери, %","Разб., %","Укрыв. теор., м²/л","Укрыв. практ., м²/л","Расход теор., кг/м²","Расход практ., кг/м²","Расход практ., л/м²","Стоимость, руб/м²","Количество на объект, кг","Стоимость на объект, руб"]; _write_header_row(ws,row,headers); row+=1
        for i,lr in enumerate(result.layers):
            fill=ALT_FILL if i%2 else None; binder=lr.material.binder_type.value if hasattr(lr.material.binder_type,"value") else str(lr.material.binder_type); values=[i+1,lr.material.material_name,binder,lr.target_dft,lr.wft,lr.losses_percent,lr.thinner_percent,lr.theoretical_coverage,lr.practical_coverage,lr.theoretical_consumption_kg,lr.practical_consumption_kg,lr.practical_consumption_l,lr.cost_per_m2+lr.thinner_cost_per_m2,lr.total_consumption_kg,lr.total_cost]
            for col,value in enumerate(values,1): _cell(ws,row,col,value,fill=fill)
            row+=1
        _cell(ws,row,2,"ИТОГО",True,TOTAL_FILL,LEFT); _cell(ws,row,4,result.total_dft,True,TOTAL_FILL); _cell(ws,row,11,result.total_practical_consumption_kg,True,TOTAL_FILL); _cell(ws,row,12,result.total_practical_consumption_l,True,TOTAL_FILL); _cell(ws,row,13,result.total_cost_per_m2,True,TOTAL_FILL); _cell(ws,row,14,result.total_practical_consumption_kg*(result.object_data.area_m2 or 0),True,TOTAL_FILL); _cell(ws,row,15,result.total_cost,True,TOTAL_FILL); ws.freeze_panes="A4"; _auto_width(ws,8,28)
    def _sheet_materials(self,ws:Worksheet,result:SystemCalculationResult)->None:
        ws.cell(1,1,"Спецификация материалов").font=TITLE_FONT; row=3; _write_header_row(ws,row,["Материал","Производитель","Плотность, кг/л","Сухой остаток, %","Цена, руб/кг","Цена, руб/л","Количество, кг","Количество, л","Стоимость, руб"]); row+=1
        for i,lr in enumerate(result.layers):
            fill=ALT_FILL if i%2 else None; values=[lr.material.material_name,lr.material.manufacturer or "—",lr.material.density,lr.material.solids_by_volume_percent if lr.material.solids_by_volume_percent is not None else lr.material.solids_percent,lr.material.price_per_kg,lr.material.price_per_liter,lr.total_consumption_kg,lr.total_consumption_l,lr.total_cost]
            for col,value in enumerate(values,1): _cell(ws,row,col,value,fill=fill)
            row+=1
        _auto_width(ws)
    def _sheet_summary(self,ws:Worksheet,result:SystemCalculationResult)->None:
        row=self._title_block(ws,"Итоговый расчёт",result.object_data); ws.cell(row,1,"Сводка").font=SUBTITLE_FONT; row+=1; items=[("Система",result.system.system_name or "Пользовательская"),("Количество слоёв",len(result.layers)),("Общая толщина DFT, мкм",result.total_dft),("Расход теоретический, кг/м²",result.total_theoretical_consumption_kg),("Расход практический, кг/м²",result.total_practical_consumption_kg),("Расход практический, л/м²",result.total_practical_consumption_l),("Стоимость материала, руб/м²",result.total_cost_per_m2),("Стоимость объекта, руб",result.total_cost),("В т.ч. разбавитель, руб",result.total_thinner_cost)]
