...
class CustomerExcelExporter(ExcelExporter):
    ...
    def __init__(self, settings: Optional[AppSettings] = None): super().__init__(settings)
    @property
    ...
    @staticmethod
    def _binder(value) -> str: return value.value if hasattr(value, "value") else str(value or "")
    @staticmethod
    def _copy_row_style(ws, source_row: int, target_row: int, min_col: int = 2, max_col: int = 18) -> None:
        ws.row_dimensions[target_row].height = ws.row_dimensions[source_row].height; ws.row_dimensions[target_row].hidden = ws.row_dimensions[source_row].hidden
    ...
    def export_calculation(self, result: SystemCalculationResult, path: str | Path, recommendation: Optional[RecommendationResult] = None) -> Path:
        if self.report_mode == "engineering": return EngineeringExcelExporter().export_system(result, path)
