...
class CustomerExcelExporter(ExcelExporter):
    """Заполняет одну знакомую заказчику таблицу ``База``."""
    ...
    def _prepare_base_sheet(self, wb, layer_count: int):
        if "База" not in wb.sheetnames:
            return None
        ws = wb["База"]
        # The historical workbook had a duplicated kg/l block. v3 keeps only
        # the first table and removes the legacy duplicate section.
        if ws.max_row > 11:
            ws.delete_rows(12, ws.max_row - 11)
        if layer_count > 2:
            self._expand_section(ws, self._SECTION["layer_start"], self._SECTION["thinner_start"], self._SECTION["total_row"], layer_count)
        return ws
