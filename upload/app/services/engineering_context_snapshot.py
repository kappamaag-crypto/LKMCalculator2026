    ...
    return {
        ...
        "rules": {
            rule_id: {
                ...
                "value": rule.value,
                "status": rule.status,
                "source": _source_to_dict(rule.source),
                "applicability": rule.applicability,
                "notes": rule.notes,
            }
            for rule_id, rule in model.rules.items()
        },
    }
...
def normative_model_from_dict(data: dict[str, Any] | None) -> NormativeModel | None:
    if not data:
        return None
    rules = {
        rule_id: NormativeRule(
            rule_id=str(rule_data.get("rule_id", rule_id)),
            value=rule_data.get("value"),
            status=str(rule_data.get("status", UNKNOWN)),
            source=_source_from_dict(rule_data.get("source")),
            applicability=str(rule_data.get("applicability", "")),
            notes=str(rule_data.get("notes", "")),
        )
        for rule_id, rule_data in dict(data.get("rules", {})).items()
    }
...
def surface_condition_to_dict(condition: SurfaceCondition | None) -> dict[str, Any] | None:
    ...
    return {
        "preparation": {
            "method": condition.preparation.method,
            "grade": condition.preparation.grade,
            "standard": _source_to_dict(condition.preparation.standard),
            "assessment": condition.preparation.assessment,
            "notes": condition.preparation.notes,
        },
        "profile": {
            "measurement": condition.profile.measurement,
            "minimum_um": condition.profile.minimum_um,
            "nominal_um": condition.profile.nominal_um,
            "maximum_um": condition.profile.maximum_um,
            "standard": _source_to_dict(condition.profile.standard),
            "assessment": condition.profile.assessment,
            "notes": condition.profile.notes,
        },
        "substrate": condition.substrate,
        "contamination_status": condition.contamination_status,
        "moisture_status": condition.moisture_status,
    }
...
def surface_condition_from_dict(data: dict[str, Any] | None) -> SurfaceCondition | None:
    if not data:
        return None
    preparation = dict(data.get("preparation", {}))
    profile = dict(data.get("profile", {}))
    return SurfaceCondition(
        preparation=SurfacePreparation(
            method=preparation.get("method", "UNKNOWN"),
            grade=str(preparation.get("grade", "")),
            standard=_source_from_dict(preparation.get("standard")),
            assessment=str(preparation.get("assessment", UNKNOWN)),
            notes=str(preparation.get("notes", "")),
        ),
        profile=SurfaceProfile(
            measurement=profile.get("measurement", "UNKNOWN"),
            minimum_um=profile.get("minimum_um"),
            nominal_um=profile.get("nominal_um"),
            maximum_um=profile.get("maximum_um"),
            standard=_source_from_dict(profile.get("standard")),
            assessment=str(profile.get("assessment", UNKNOWN)),
