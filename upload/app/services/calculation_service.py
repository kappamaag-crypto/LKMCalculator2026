...
class CalculationService:
    ...
    def __init__(self, calculator: Optional[SystemCalculator] = None, comparison_engine: Optional[ComparisonEngine] = None, compatibility_engine: Optional[LayerCompatibilityEngine] = None, default_losses: float = 0.0):
    ...
    def calculate_layer(self, material: Material, target_dft: float, losses_percent: float = 0.0, thinner_percent: float = 0.0, thinner: Optional[Material] = None, thinner_basis: Optional[str] = None, area_m2: float = 1.0):
        return LayerCalculator.calculate(material=material, target_dft=target_dft, losses_percent=losses_percent, thinner_percent=thinner_percent, thinner_basis=thinner_basis, thinner=thinner, area_m2=area_m2)

    def calculate_system(self, obj: ObjectData, layers: Sequence[LayerInput], system: Optional[CoatingSystem] = None, engineering_context: EngineeringContext | None = None) -> tuple[SystemCalculationResult, ValidationResult]:
        return self.calculator.calculate(obj, layers, system=system, engineering_context=engineering_context)

    def calculate_from_template(self, obj: ObjectData, template, materials_by_id=None, engineering_context: EngineeringContext | None = None):
        return self.calculator.calculate_from_template(obj, template, materials_by_id=materials_by_id, engineering_context=engineering_context)

    def compare_systems(self, obj: ObjectData, systems: Sequence[CoatingSystem], materials_by_id=None) -> ComparisonResult:
    ...
    def check_system_tds_dft(self, system, materials_by_id=None):
        ...
        for layer in system.layers:
            mat = layer.material if layer.material is not None else materials_by_id.get(layer.material_id) if layer.material_id is not None else None
            if mat is not None:
                findings.append(check_target_dft_with_known_tds(mat, layer.target_dft or layer.dft_min or layer.dft_max))
        return findings
    ...
    def check_chemical_resistance(self, material_names: Sequence[str], agents: Sequence[ChemicalAgent], *, require_known: bool = False, extra_rules: Sequence | None = None) -> ChemicalResistanceCheckResult:
        rules = list(list_known_chemical_resistance_rules())
        if extra_rules:
            rules.extend(extra_rules)
        return check_chemical_resistance(material_names, agents, rules, require_known=require_known)

    def run_pre_application_check(self, obj: ObjectData, materials: Sequence[Material], *, surface_condition: SurfaceCondition | None = None, actual_dfts: Sequence[float | None] | None = None, engineering_context: EngineeringContext | None = None) -> PreApplicationCheckResult:
    ...
    def format_summary(self, result: SystemCalculationResult) -> str:
        ...
        lines = [f"Система: {result.system.system_name}", f"Слоёв: {len(result.layers)}", f"Суммарная DFT: {result.total_dft:g} мкм", f"Стоимость / м²: {cost_m2_text}"]
        if thinner_l_m2:
        ...
        if len(result.layers) >= 2:
            ...
            status_labels = {"разрешено": "РАЗРЕШЕНО", "предупреждение": "ПРЕДУПРЕЖДЕНИЕ", "запрещено": "ЗАПРЕЩЕНО", "нет подтвержденных данных": "UNKNOWN — НЕТ ПОДТВЕРЖДЁННЫХ ДАННЫХ"}
            lines.append("Совместимость слоёв: " + status_labels.get(compatibility.status.value, compatibility.status.value))
