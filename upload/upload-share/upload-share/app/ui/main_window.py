...
from app.ui.views.book_search_view import BookSearchView
from app.ui.views.inspection_view import InspectionView
from app.ui.views.system_catalog_review_view import SystemCatalogReviewView
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        menubar=self.menuBar();file_menu=menubar.addMenu("Файл");act_exit=QAction("Выход",self);act_exit.triggered.connect(self.close);file_menu.addAction(act_exit);engineering_menu=menubar.addMenu("Инженерное");act_context=QAction("Контекст НД и поверхности…",self);act_context.triggered.connect(self._edit_engineering_context);engineering_menu.addAction(act_context);act_precheck=QAction("Pre-Application Check…",self);act_precheck.triggered.connect(self._show_pre_application_check);engineering_menu.addAction(act_precheck);act_chem=QAction("Химстойкость…",self);act_chem.triggered.connect(self._show_chemical_resistance);engineering_menu.addAction(act_chem);act_explanation=QAction("Пояснение расчёта…",self);act_explanation.triggered.connect(self._show_calculation_explanation);engineering_menu.addAction(act_explanation);act_books=QAction("Инженерная БД…",self);act_books.triggered.connect(lambda:self.tabs.setCurrentWidget(self.book_search_view));engineering_menu.addAction(act_books);act_catalog=QAction("Каталог систем 1–4…",self);act_catalog.triggered.connect(lambda:self.tabs.setCurrentWidget(self.catalog_review_view));engineering_menu.addAction(act_catalog);act_inspection=QAction("Инспекция / контроль качества…",self);act_inspection.triggered.connect(lambda:self.tabs.setCurrentWidget(self.inspection_view));engineering_menu.addAction(act_inspection);help_menu=menubar.addMenu("Справка");act_about=QAction("О программе",self);act_about.triggered.connect(self._on_about);help_menu.addAction(act_about);self.setStatusBar(QStatusBar())
    def _edit_engineering_context(self):
        ...
        self._engineering_context=context;self.calc_view.set_engineering_context(context);self.scenario_view.set_engineering_context(context)
    ...
    def _show_pre_application_check(self):
        result=getattr(self,"_last_calculation_result",None)
        if result is None:
            QMessageBox.information(self,"Pre-Application Check","Сначала выполните расчёт в разделе «Расчёт».");self.tabs.setCurrentWidget(self.calc_view);return
        PreApplicationDialog(self.calc_service,result,self).exec()
    def _show_chemical_resistance(self):
        ChemicalResistanceDialog(self.calc_service,self._materials,self).exec()
    def _show_calculation_explanation(self):
        result=getattr(self,"_last_calculation_result",None)
        if result is None:
            QMessageBox.information(self,"Пояснение расчёта","Сначала выполните расчёт в разделе «Расчёт».");self.tabs.setCurrentWidget(self.calc_view);return
        report=self.calc_service.explain_calculation(result);ExplanationDialog(report,self).exec()
    def _load_scenario_templates(self):return self.template_service.load_systems(self._materials)
    def _load_demo_data(self):self.calc_view.set_materials(self._materials);self.calc_view.set_systems(self._systems);self.scenario_view.set_systems(self._load_scenario_templates());self.rec_view.set_systems(self._systems)
    def _refresh_system_catalog(self):self._systems=_load_systems_from_db(self._materials,_demo_systems());self.calc_view.set_systems(self._systems);self.scenario_view.set_systems(self._load_scenario_templates());self.rec_view.set_systems(self._systems)
    ...
    def _on_settings_changed(self,settings):self.settings=settings;self.calc_view.set_settings(settings);self.statusBar().showMessage("Настройки применены к расчёту и экспорту",7000)
    @staticmethod
    ...
    def _on_material_added_from_calculation(self,material):materials=_load_materials_from_db(self._materials);self._on_materials_changed(materials);self.statusBar().showMessage(f"Материал «{material.display_name()}» добавлен в базу и доступен во всех разделах",7000)
