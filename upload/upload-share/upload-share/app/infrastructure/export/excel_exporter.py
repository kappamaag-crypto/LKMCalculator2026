...
class ExcelExporter:
    ...
    def _sheet_comparison(self,ws:Worksheet,comparison:ComparisonResult)->None:
        """Полная инженерная таблица сравнения без ограничения двумя слоями."""
        ...
        summary_rows=[
            ...
            ("Теоретический расход ЛКМ, кг/м²",[s.total_theoretical_consumption_kg for s in systems]),
            ("Практический расход ЛКМ, кг/м²",[s.total_practical_consumption_kg for s in systems]),
            ("Практический расход ЛКМ, л/м²",[s.total_practical_consumption_l for s in systems]),
            ("Расход разбавителя, кг/м²",[sum(lr.thinner_consumption_kg for lr in s.layers) for s in systems]),
            ("Расход разбавителя, л/м²",[sum(lr.thinner_consumption_l for lr in s.layers) for s in systems]),
            ("Итого стоимость, руб/м²",[_display(s.total_cost_per_m2) for s in systems]),
            ("Стоимость объекта, руб",[_display(s.total_cost,0) for s in systems]),
        ]
        ...
        layer_indicators=(
            ("Материал",lambda lr: lr.material.display_name()),
            ("Связующее",lambda lr: lr.material.binder_type.value if hasattr(lr.material.binder_type,"value") else str(lr.material.binder_type)),
            ("DFT, мкм",lambda lr: lr.target_dft),
            ("WFT, мкм",lambda lr: lr.wft),
            ("Потери, %",lambda lr: lr.losses_percent),
            ("Разбавление, %",lambda lr: lr.thinner_percent),
            ("Разбавитель",lambda lr: lr.thinner.display_name() if lr.thinner else "—"),
            ("Укрывистость теор., м²/л",lambda lr: lr.theoretical_coverage),
            ("Укрывистость практ., м²/л",lambda lr: lr.practical_coverage),
            ("Расход теор., кг/м²",lambda lr: lr.theoretical_consumption_kg),
            ("Расход практ., кг/м²",lambda lr: lr.practical_consumption_kg),
            ("Расход практ., л/м²",lambda lr: lr.practical_consumption_l),
            ("Разбавитель, кг/м²",lambda lr: lr.thinner_consumption_kg),
            ("Разбавитель, л/м²",lambda lr: lr.thinner_consumption_l),
            ("Стоимость материала, руб/м²",lambda lr: _display(lr.cost_per_m2)),
            ("Стоимость разбавителя, руб/м²",lambda lr: _display(lr.thinner_cost_per_m2)),
            ("Итого по слою, руб/м²",lambda lr: _display(_cost_add(lr.cost_per_m2,lr.thinner_cost_per_m2))),
        )
        for layer_index in range(max_layers):
            for indicator,getter in layer_indicators:
                values=[]
                for s in systems:
