...
class ComparisonView(QWidget):
    ...
    def _fill_table(self,comparison:ComparisonResult):
        ...
        rows=[("Количество слоёв",[len(s.layers) for s in systems],False),("Общая толщина, мкм",[s.total_dft for s in systems],False),("Расход ЛКМ, кг/м²",[s.total_practical_consumption_kg for s in systems],True),("Расход ЛКМ, л/м²",[s.total_practical_consumption_l for s in systems],True),("Стоимость ЛКМ, руб/м²",paint_m2,True),("Стоимость ЛКМ на объект, руб",[(s.total_cost-(s.total_thinner_cost or 0)) if s.total_cost is not None else None for s in systems],True),("Разбавитель, руб/м²",thinner_m2,True),("ЛКМ + разбавитель, руб/м²",[s.total_cost_per_m2 for s in systems],True)]
