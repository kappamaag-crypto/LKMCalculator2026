...
class TestScorer:
    ...
    def test_unknown_cost_is_safe(self):
        ...
        assert fr.passed
        breakdown=score_system(fr,obj,calc_result=None,all_costs=[])
        assert breakdown.cost == 50.0
...
class TestRecommender:
    ...
    def test_unknown_cost_does_not_break_recommendation(self):
        systems=[make_system("Known cost",[CorrosionCategory.C4],"High",price=500.0),make_system("Unknown cost",[CorrosionCategory.C4],"High",price=None)]
        obj=ObjectData(corrosion_category=CorrosionCategory.C4,durability=DurabilityLevel.HIGH,surface_type=SurfaceType.NEW_STEEL,environment=EnvironmentType.OUTDOOR)
        result=RecommendationEngine().recommend(obj,systems,calculate_costs=True,top_n=5)
        assert len(result.items)==2
        unknown=next(i for i in result.items if i.system.system_name=="Unknown cost")
        assert unknown.status=="Подходит"
        assert unknown.score>=0
        assert any("Стоимость не рассчитана" in x for x in unknown.reasons)
