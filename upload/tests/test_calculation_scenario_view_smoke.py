...
def make_result(system: CoatingSystem) -> SystemCalculationResult:
    return SystemCalculationResult(
        system=system,
        object_data=ObjectData(area_m2=1.0),
        layers=[
...
def test_scenario_view_surfaces_compatibility_status_and_notes():
    ...
    view.table_systems.item(0, 0).setCheckState(view.table_systems.item(0, 0).checkState().Checked)
    view._calculate()
