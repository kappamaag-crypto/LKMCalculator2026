...
class TestLayerCalculator:
    def test_primer_basic(self):
        ...
        assert result.theoretical_coverage == pytest.approx(10 * 73 / 200)
        assert result.practical_consumption_kg == pytest.approx(200 / 10 / 73 * 1.4)
    ...
    def test_packages(self):
        ...
        assert result.packages_count == 20
...
class TestSystemCalculator:
    def test_two_layer_system(self):
        ...
        layers = [LayerInput(material=make_primer(), target_dft=200.0, losses_percent=0.0), LayerInput(material=make_finish(), target_dft=100.0, losses_percent=0.0)]
        result, validation = SystemCalculator().calculate(obj, layers)
        assert not validation.has_errors
        assert result.total_dft == 300.0
        assert len(result.layers) == 2
        assert result.total_cost_per_m2 > 0
        assert result.total_cost == pytest.approx(result.total_cost_per_m2 * 100)

    def test_area_from_elements(self):
        obj = ObjectData(area_per_element=50.0, elements_count=4)
        result, _ = SystemCalculator().calculate(obj, [LayerInput(material=make_primer(), target_dft=200.0)])
        assert result.layers[0].total_consumption_kg == pytest.approx(result.layers[0].practical_consumption_kg * 200)


class TestValidation:
    def test_negative_area(self):
        r = validate_object_data(ObjectData(area_m2=-10))
        assert r.has_errors
        assert any(i.code == "OBJ_AREA_NEG" for i in r.errors)

    def test_dft_above_max(self):
        r = validate_layer_input(make_primer(), target_dft=250.0)
        assert r.has_warnings
        assert any(i.code == "LAYER_DFT_ABOVE_MAX" for i in r.warnings)

    def test_solids_over_100(self):
        mat = make_primer(); mat.solids_percent = 120.0
    ...
    def test_losses_ge_100(self):
        assert validate_layer_input(make_primer(), target_dft=100.0, losses_percent=100.0).has_errors

    def test_dew_point(self):
        r = validate_object_data(ObjectData(area_m2=10, surface_temperature=5.0, dew_point=8.0))
        assert r.has_errors
        assert any(i.code == "OBJ_DEW_POINT" for i in r.errors)

    def test_before_calculation_blocks_on_error(self):
        mat = make_primer(); mat.density = -1.0
        assert validate_before_calculation(ObjectData(area_m2=10), [(mat, 200.0, 0.0, 0.0)]).has_errors


class TestComparison:
    def test_compare_two_systems(self):
        from app.domain.comparison import ComparisonEngine
        primer, finish, obj = make_primer(), make_finish(), ObjectData(area_m2=100.0)
        comparison = ComparisonEngine().compare(obj, [("Система А", [LayerInput(material=primer, target_dft=200.0), LayerInput(material=finish, target_dft=100.0)]), ("Система Б", [LayerInput(material=primer, target_dft=150.0), LayerInput(material=finish, target_dft=80.0)])])
