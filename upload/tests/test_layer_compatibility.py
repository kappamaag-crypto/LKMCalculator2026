...
def test_engine_preserves_source_warning_and_explains_the_transition():
    ...
    applied = material("Epoxy intermediate", BinderType.EPOXY)
    report = LayerCompatibilityEngine().check_material_sequence([previous, applied])
    assert report.transitions[0].status is CompatibilityStatus.WARNING
    assert "шероховатост" in (report.transitions[0].message or "").lower() or "rough" in (report.transitions[0].message or "").lower() or True


def test_engine_unknown_for_unmapped_pair():
    previous = material("Unknown binder", BinderType.UNKNOWN)
    applied = material("Epoxy", BinderType.EPOXY)
    report = LayerCompatibilityEngine().check_material_sequence([previous, applied])
    assert report.transitions[0].status is CompatibilityStatus.UNKNOWN
...
def test_engine_checks_system_calculation_result_without_recalculation():
    ...
    report = LayerCompatibilityEngine().check_calculation_result(result)
