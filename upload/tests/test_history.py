...
def sample_result():
    ...
    result, val = CalculationService().calculate_system(obj, [LayerInput(material=primer, target_dft=100)])
...
def test_save_and_list(sample_result):
    with tempfile.TemporaryDirectory() as tmp:
        ...
        engine = get_engine(db); init_db(engine); sf = get_session_factory(engine)
...
def test_unknown_cost_is_preserved_as_null(sample_result):
    sample_result.layers[0].material.price_per_kg = None
    sample_result.total_cost_per_m2 = None
    sample_result.total_cost = None
    with tempfile.TemporaryDirectory() as tmp:
        db = Path(tmp) / "unknown_cost.sqlite"
        engine = get_engine(db); init_db(engine); sf = get_session_factory(engine)
        with session_scope(sf) as session:
        ...
        with session_scope(sf) as session:
            calc = HistoryService(session).get_calculation(calc_id)
            assert calc is not None
            assert calc.total_cost is None
            assert calc.total_cost_per_m2 is None


def test_snapshot_contains_thinner_information():
    primer = Material(material_name="Грунт 2K", material_type=MaterialType.PRIMER_ENAMEL, binder_type=BinderType.EPOXY, density=1.4, solids_percent=73.0, price_per_kg=500.0)
    ...
    obj = ObjectData(object_name="Тест разбавителя", area_m2=10.0)
    result, val = CalculationService().calculate_system(obj, [LayerInput(material=primer, target_dft=100, thinner_percent=10, thinner=thinner)])
    assert not val.has_errors
    with tempfile.TemporaryDirectory() as tmp:
        db = Path(tmp) / "thinner.sqlite"
        engine = get_engine(db); init_db(engine); sf = get_session_factory(engine)
        with session_scope(sf) as session:
        ...
        with session_scope(sf) as session:
            ...
            assert layer["thinner_name"] == "Разбавитель Test"
            assert layer["thinner_density"] == 0.9
            assert layer["thinner_price_per_kg"] is None
            assert layer["thinner_percent"] == 10
            assert layer["thinner_consumption_l"] > 0
