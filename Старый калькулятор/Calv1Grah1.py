...
from typing import List, Dict, Any, Optional, Union, Sequence, TypedDict, cast
...
class SystemDict(TypedDict):
    """Тип словаря данных системы покрытия"""
    name: str
    layers_count: int
    total_cost: float
...
from utils import safe_float, safe_str, is_thinner_name

# Создаем новый тип для индексов слояfgrrhrhrth
...
def create_empty_layer_dict() -> LayerDict:
    """Создает пустой словарь слоя"""
    return LayerDict(
        name="",
        binder=None,
        ral=None,
...
def validate_layer_with_size(layer: Optional[LayerList], min_size: int = 16) -> bool:
    """
    Проверяет валидность слоя с проверкой минимального размера
    
    Args:
        layer: Список значений слоя
        min_size: Минимальный размер списка
        
    Returns:
        bool: True если слой валиден
    """
    if not layer or len(layer) < min_size:
        return False
    return validate_layer(layer)

# Аликлас для обратной совместимости
Layer = LayerList

def create_layer_dict(layer: LayerList) -> LayerDict:
    """Создает словарь с данными слоя"""
    if not layer or len(layer) < 16:
        return create_empty_layer_dict()
        
    name = safe_str(layer[LayerIndex.NAME]) or ""
    binder = safe_str(layer[LayerIndex.BINDER])
    ral = safe_str(layer[LayerIndex.RAL])
    dry_thickness = safe_float(layer[LayerIndex.DRY_THICKNESS])
    consumption_kg = safe_float(layer[LayerIndex.THEOR_CONSUMPTION_KG])
    density = safe_float(layer[LayerIndex.DENSITY])
    consumption_l = consumption_kg / density if density > 0 else 0.0
    cost = safe_float(layer[LayerIndex.COST])
...
def create_layer_data_list(layers: Sequence[LayerList]) -> List[LayerDict]:
    """Создает список структур данных слоев, исключая разбавители"""
    return [
        create_layer_dict(layer) 
        for layer in layers 
        if validate_layer(layer)
    ]

ComparisonResults = Dict[str, Union[str, float, int]]
...
class ComboboxWithSearch(ttk.Combobox):
    ...
    def __init__(self, parent: tk.Widget, values: List[str], **kwargs: Any) -> None:
...
class HistoryManager:
    ...
    def __init__(self, main_app: Optional['ExcelExporterApp'] = None) -> None:
        self.history_file: str = 'calculation_history.json'
        self.comparison_history_file: str = 'comparison_history.json'
        self.main_app: Optional['ExcelExporterApp'] = main_app
        self.calculation_history: List[CalculationHistoryEntry] = []
        self.comparison_history: List[ComparisonHistoryEntry] = []
        self.master: Optional[tk.Tk] = getattr(main_app, 'master', None) if main_app else None
        self.load_history()
        ...
        if os.path.exists(self.history_file):
            try:
                with open(self.history_file, 'r', encoding='utf-8') as f:
                    history = json.load(f)
                    # Проверяем и корректируем каждую запись
                    ...
                    for entry in history:
                        try:
                            ...
                            # Корректируем date_display если нужно
                            if not entry.get('date_display') or entry['date_display'] == '0':
                                try:
                                    dt = datetime.fromisoformat(entry['timestamp'])
                                    entry['date_display'] = dt.strftime('%d.%m.%Y %H:%M:%S')
                                except:
                                    # Если не удалось распарсить timestamp, используем текущую дату
                                    now = datetime.now()
                                    entry['timestamp'] = now.isoformat()
                                    entry['date_display'] = now.strftime('%d.%m.%Y %H:%M:%S')
    ...
    def _convert_layer_to_dict(self, layer: LayerList) -> LayerDict:
        """Конвертирует слой в структуру данных для истории."""
        return create_layer_dict(layer)

    def add_calculation(self, layers: Sequence[LayerList], total_cost: float, total_thickness: float,
                     total_consumption_kg: float, total_consumption_l: float) -> None:
        """
        ...
        Args:
            ...
            total_thickness: Общая толщина
            total_consumption_kg: Общий расход в кг
            total_consumption_l: Общий расход в л
        """
        try:
            now = datetime.now()
            main_layers = [layer for layer in layers if validate_layer(layer)]
            
            if not main_layers:
                logging.warning("Нет валидных слоев для сохранения")
                return
                
            layer_dicts: List[LayerDict] = []
            for layer in main_layers:
                if validate_layer_with_size(layer):
                    try:
                        layer_dict = self._convert_layer_to_dict(layer)
                        layer_dicts.append(layer_dict)
                    except Exception as e:
                        logging.error(f"Ошибка при конвертации слоя: {e}")
            
            if not layer_dicts:
                logging.warning("Не удалось сконвертировать слои в словари")
                return
                
            entry = CalculationHistoryEntry(
                timestamp=now.isoformat(),
                date_display=now.strftime('%Y-%m-%d %H:%M:%S'),
                layers_count=len(layer_dicts),
                total_cost=total_cost,
                total_thickness=total_thickness,
                total_consumption_kg=total_consumption_kg,
                total_consumption_l=total_consumption_l,
                layer_names=[str(d.get('name', '')) for d in layer_dicts],
                layers=layer_dicts
            )
    ...
    def add_comparison(self, systems_data: Sequence[SystemList], system_names: Sequence[str],
                     comparison_results: ComparisonResults) -> None:
        """
        Добавляет запись в историю сравнений
        
        Args:
            systems_data: Список систем покрытия
            system_names: Список названий систем
            comparison_results: Результаты сравнения систем
        """
        try:
            now = datetime.now()
            systems: List[SystemDict] = []
            
            # Обрабатываем данные по каждой системе
            for name, system in zip(system_names, systems_data):
                # Фильтруем только валидные слои, исключая разбавители
                valid_layers = [layer for layer in system if validate_layer(layer)]
                
                if valid_layers:
                    systems.append(SystemDict(
                        name=name,
                        layers_count=len(valid_layers),
                        total_cost=self.calculate_system_cost(valid_layers),
                        total_thickness=self.calculate_system_thickness(valid_layers),
                        layers=create_layer_data_list(valid_layers)
                    ))
            ...
            comparison: ComparisonHistoryEntry = {
                ...
                'best_system': comparison_results['best_system'],
                'best_cost': comparison_results['best_cost'],
                'economy_percent': comparison_results['economy_percent'],
                'systems': systems
            }
            ...
            # Ограничиваем историю последними 5000 записями
            if len(self.comparison_history) > 5000:
                self.comparison_history = self.comparison_history[-5000:]
    ...
    def calculate_system_cost(self, system: Sequence[LayerList]) -> float:
        """
        Вычисляет общую стоимость системы
        
        Args:
            system: Список слоев системы
        
        Returns:
            float: Общая стоимость системы
        """
        try:
            return sum(
                safe_float(layer[LayerIndex.COST])
                for layer in system 
                if validate_layer(layer) and not is_thinner_name(layer[LayerIndex.NAME])
            )
        except Exception as e:
            logging.error(f"Ошибка при расчете стоимости системы: {str(e)}")
            return 0.0

    def calculate_system_thickness(self, system: Sequence[LayerList]) -> float:
        """
        Вычисляет общую толщину покрытия
        
        Args:
            system: Список слоев системы
        
        Returns:
            float: Общая толщина покрытия
        """
        try:
            return sum(
                safe_float(layer[LayerIndex.DRY_THICKNESS])
                for layer in system
                if validate_layer(layer) and not is_thinner_name(layer[LayerIndex.NAME])
            )
        except Exception as e:
            logging.error(f"Ошибка при расчете толщины системы: {str(e)}")
            return 0.0
    
    def get_calculation_history(self) -> List[CalculationData]:
...
class ComparisonHistoryDialog:
    ...
    def load_history(self):
        ...
        for comparison in history:
            ...
            system_names = [system.get('name', '') for system in comparison.get('systems', [])]
            if system_names:
    ...
    def create_summary_tab(self, parent, comparison):
        ...
        # Заполняем данными
        for system_data in comparison['systems']:
    ...
    def load_system_to_calculation(self):
        ...
        def load_selected_system():
            ...
            system_data = comparison['systems'][system_index]
...
class SystemComparisonDialog:
    ...
    def export_layers_details(self, ws, start_row, system_params):
        ...
        for i, (system_name, params) in enumerate(zip(self.system_names, system_params)):
            ...
            for thinner in params['thinner_layers']:
                display_name = self.format_thinner_for_export(thinner[self.NAME], thinner[self.THINNER_PERCENT])
                row_data = [
                    system_name,
                    'Растворитель',
    ...
    def format_thinner_for_export(self, thinner_full_name, actual_percent = None):
        ...
        try:
            ...
            if '(' in thinner_full_name and ')' in thinner_full_name:
                ...
                if len(bracket_parts) >= 2:
                    ...
                    # ИСПОЛЬЗУЕМ АКТУАЛЬНЫЙ ПРОЦЕНТ ИЗ СЛОЯ, А НЕ ПАРСИМ ИЗ НАЗВАНИЯ!
                    if actual_percent is not None and actual_percent > 0:
                        percentage = f"{actual_percent}%"
                    else:
                    ...
                    # Форматируем для экспорта: "Разбавитель Ксилол 12% для Бланк"
...
"""
    ...
    def calculate_thinner_consumption(self, thinner_layer, parent_layer):
        ...
        Использует ТОЛЬКО процент указанный пользователем в слое растворителя
        """
        try:
            logging.info(f"Расчет растворителя: {thinner_layer[self.NAME]}")
            logging.info(f"Родительский слой: {parent_layer[self.NAME]}")
            
            # ИСПРАВЛЕНО: Используем ТОЛЬКО процент который сохранен в слое, не меняем на значение по умолчанию
            if thinner_layer[self.THINNER_PERCENT] is not None and thinner_layer[self.THINNER_PERCENT] > 0:
                thinner_percent = thinner_layer[self.THINNER_PERCENT] / 100.0
                logging.info(f"Используется процент растворителя указанный пользователем: {thinner_percent*100}%")
            else:
                # Если процент не указан, берем 5% ТОЛЬКО при первом создании
                thinner_percent = 0.05
                logging.info(f"Процент не указан, используется значение по умолчанию: 5%")
                
            logging.info(f"Итоговый процент растворителя: {thinner_percent*100}%")
    ...
    def open_edit_dialog(self, index, parent_dialog):
        ...
        # Добавляем отдельное поле для редактирования процента растворителя
        if is_thinner:
            # Добавляем разделитель
            ttk.Label(main_frame_tab, text="").grid(row=len(fields), column=0, padx=5, pady=5)
            
            ttk.Label(main_frame_tab, text="Процент растворителя (%):", foreground="blue", font=("Arial", 9, "bold")).grid(row=len(fields)+1, column=0, padx=5, pady=5, sticky='w')
            thinner_percent_entry = ttk.Entry(main_frame_tab, width=30)
            thinner_percent_entry.grid(row=len(fields)+1, column=1, padx=5, pady=5, sticky='ew')
            
            # Заполняем текущим значением
            current_percent = self.layers[index][self.THINNER_PERCENT] if self.layers[index][self.THINNER_PERCENT] is not None else 5
            thinner_percent_entry.insert(0, str(current_percent))
            entries['Процент растворителя'] = thinner_percent_entry
        ...
        def save_changes():
            ...
            try:
                if is_thinner:
                    ...
                    # Обновляем процент растворителя если он был изменен
                    if 'Процент растворителя' in entries:
                        try:
                            new_percent = float(entries['Процент растворителя'].get() or 5)
                            updated_layer[self.THINNER_PERCENT] = new_percent
                            logging.info(f"Обновлен процент растворителя: {new_percent}%")
                        except ValueError:
                            pass
                    
                    # Пересчитываем растворитель заново с новым процентом
                    parent_layer = self.find_parent_layer_for_thinner(original_name)
                    if parent_layer:
                        updated_layer = self.calculate_thinner_consumption(updated_layer, parent_layer)
    ...
    def save_layer(self, entries, coating_combo, parent_combo, thinner_combo, thinner_entries, thinner_percent):
        ...
        try:
            ...
            if current_tab == 0:  # Основное покрытие
                ...
                # Получаем пользовательский процент растворителя
                user_thinner_percent = float(thinner_percent.get() or 5)
                
                # Создаем слой растворителя с ПОЛЬЗОВАТЕЛЬСКИМ значением процента в названии
                thinner_layer = [
                    f"Растворитель {parent_layer_name} ({thinner_name} {user_thinner_percent}%)",
                    '',  # связующее
                    '',  # RAL
                    ...
                    0.0,  # расход с учетом потерь кг/м2
                    0.0,  # стоимость
                    user_thinner_percent  # процент растворителя
                ]
    ...
    def force_recalculate_thinners(self):
        """Алиас для принудительного пересчета растворителей (обратная совместимость)"""
        self.recalculate_all_thinners()
