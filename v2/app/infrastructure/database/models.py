...
class CoatingSystemORM(Base):
    ...
    layers: Mapped[list["CoatingSystemLayerORM"]] = relationship(
        ...
        order_by="CoatingSystemLayerORM.layer_number",
    )

    def __repr__(self) -> str:
        return f"<CoatingSystemORM id={self.id} name={self.system_name!r}>"
...
class CoatingSystemLayerORM(Base):
    ...
    thinner_material_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("materials.id", ondelete="SET NULL"), nullable=True
    )
    notes: Mapped[str] = mapped_column(Text, default="")
    ...
    __table_args__ = (
        UniqueConstraint("system_id", "layer_number", name="uq_system_layer_number"),
    )


class LayerCompatibilityORM(Base):
    """Совместимость слоёв (primer → intermediate → finish)."""
    ...
    __table_args__ = (
        UniqueConstraint("from_binder", "to_binder", name="uq_binder_pair"),
    )
...
class CalculationORM(Base):
    ...
    system_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("coating_systems.id", ondelete="SET NULL"), nullable=True
    )
    ...
    # JSON snapshot of full input/result for exact restore
    ...
    layers: Mapped[list["CalculationLayerORM"]] = relationship(
        back_populates="calculation",
        cascade="all, delete-orphan",
        order_by="CalculationLayerORM.layer_number",
    )
...
class DictionaryORM(Base):
    """Справочники (производители, типы связующих и т.д.)."""
    ...
    __table_args__ = (
        UniqueConstraint("dict_type", "name", name="uq_dict_type_name"),
    )