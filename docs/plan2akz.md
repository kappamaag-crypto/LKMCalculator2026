...
| 20 | ЧАСТИЧНО | KB индексирует source identity/SHA-256 и searchable content. Добавлены `Системы 1.xls`, `Системы 2.XLSX`, `Системы 3.xlsx`, `Системы 4.xlsx`. Теперь XLS/XLSX поддерживаются поисковым индексом; добавлен review-first staging importer для строк систем и кандидатов материалов. БД напрямую не изменяется. Остаются UI acceptance и интеграция каталога. |
...
| 27 | ЧАСТИЧНО | Требуется Material Data Quality. Для `Системы 1–4` уже есть staging: распознавание колонок, candidate materials, duplicate normalization, provenance. Следующий шаг — review UI и сопоставление с БД без автоматической записи. |
| 28 | НЕ ВЫПОЛНЕНО | Управляемый `LossProfile`: способ нанесения, геометрия, условия, диапазон, источник. |
| 29 | ЧАСТИЧНО | `Системы 1–4` закреплены как исходный каталог вариантов систем и кандидатов материалов. Staging importer реализован; следующий шаг — System Templates с draft/review/save, 2/3/4+ слоями и provenance. |
...
## Реализованный code stage — KB/catalogue ingestion
...
## §20/§27/§29 — следующий этап

1. Проверить фактическую структуру листов `Системы 1–4` и сопоставить реальные колонки с моделью системы/материала.
2. Не считать первую строку заголовком автоматически, если структура этого не подтверждает.
3. Добавить UI staging/review: список строк, материал-кандидаты, найденные материалы БД, новые кандидаты и причины неоднозначности.
4. Для найденного материала использовать существующую карточку; не создавать дубль.
5. Для отсутствующего материала предлагать создание неполной карточки только после подтверждения пользователя.
6. Поля, которых нет в таблице/TDS, не заполнять догадкой.
7. Для TDS-полей использовать только проверенные документы; `SPKEFFA` читать только в режиме источника и не изменять.
8. После реализации review workflow — создать полноценные System Templates из подтверждённых строк.
...
3. Продолжить §20/§27/§29: реальная структура `Системы 1–4` → staging/review → сопоставление материалов → System Templates.
